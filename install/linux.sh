...
if [ ! -r "$TTY_PATH" ]; then
  ...
  printf '%s\n' "Download it first, then run: sh linux.sh"
  exit 1
fi
...
check_docker() {
  if ! command -v docker >/dev/null 2>&1; then
    ...
    say "Install Docker Engine for $INSTALLER_OS, then rerun this installer:"
    say "$DOCKER_INSTALL_URL"
    exit 1
  fi

  if ! docker info >/dev/null 2>&1; then
    say "Docker is installed, but the Docker daemon is not running or your user cannot access it."
    say "Start Docker, verify 'docker info' works, then rerun this installer."
    exit 1
  fi

  if ! docker compose version >/dev/null 2>&1; then
    say "Docker Compose v2 is not available as 'docker compose'."
    say "Install the Docker Compose plugin, then rerun this installer:"
...
check_platform() {
  ...
  if [ "$detected_os" != "Linux" ]; then
    say "Warning: this is the Linux installer, but this system reports '$detected_os'."
    if [ "$detected_os" = "Darwin" ]; then
      say "Consider using install/macos.sh instead."
    fi
  fi
}
