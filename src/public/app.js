...
function serviceCardHtml(s) {
  ...
  const favoriteLabel = state.favorites.includes(s.id) ? `Remove ${s.name} from favorites` : `Add ${s.name} to favorites`;
  const actions = `<button class="icon-btn" data-copy="${escapeHtml(s.url)}" title="Copy URL" aria-label="Copy ${escapeHtml(s.name)} URL">⧉</button>
      <button class="icon-btn ${state.favorites.includes(s.id) ? 'active' : ''}" data-fav="${escapeHtml(s.id)}" title="Favorite" aria-label="${escapeHtml(favoriteLabel)}">★</button>
  ...
  const actionControl = state.preferences.viewMode === 'icon'
    ? `<details class="service-actions" name="service-actions"><summary aria-label="Actions for ${escapeHtml(s.name)}" title="Actions for ${escapeHtml(s.name)}"><span aria-hidden="true">•••</span></summary><span class="actions">${actions}</span></details>`
  ...
  return `<article class="service-card ${disabled ? 'is-disabled' : ''}" data-service-id="${escapeHtml(s.id)}" ${isDraggable} style="--accent:${hexToRgba(s.accent, .1)}">
    <div><div class="card-top">${iconHtml(s.icon, s.name)}${actionControl}</div><h3 title="${escapeHtml(s.name)}">${escapeHtml(s.name)}</h3><p>${escapeHtml(s.description || 'No description provided.')}</p><a class="service-link ${showHostnames ? '' : 'link-only-overlay'}" href="${escapeHtml(s.url)}" target="_blank" rel="noopener noreferrer" aria-label="Open ${escapeHtml(s.name)}" title="Open ${escapeHtml(s.name)}"><span>${showHostnames ? escapeHtml(getHost(s.url)) : ''}</span></a></div>
