...
function adminBackupsHtml() {
  const h = state.admin.health || {};
  const c = state.admin.config || {};
  return `<div class="settings-card"><h3>Backups & storage</h3><p>Download a portable configuration backup containing settings, services, user names/roles, and plugin metadata. Password hashes, sessions, and private runtime data are not exported.</p><div class="inline-controls"><button class="primary" id="download-backup" type="button">Download config backup</button><button class="ghost" id="download-db" type="button">Download SQLite database</button></div><div class="stats-row">${statCard('Database', formatBytes(h.storage?.databaseBytes || 0))}${statCard('WAL', formatBytes(h.storage?.walBytes || 0))}</div><p><strong>Data dir:</strong> ${escapeHtml(h.storage?.dataDir || '')}</p><p><strong>Plugin dir:</strong> ${escapeHtml(h.storage?.pluginDir || '')}</p><h3>Preferred backup path (operator note)</h3><p>Specify the preferred directory on the host where you run periodic backups of this container. This is a reference note for documentation and scripts, not an active automation scheduler.</p><div class="inline-controls"><input id="scheduled-backup-location" value="${escapeHtml(c.scheduledBackupLocation || '')}" placeholder="/backups/home-lab-launcher"><button class="ghost" id="save-backup-location" type="button">Save path</button></div><h3>Restore configuration backup</h3><p>Restore settings and services from a Home Lab Launcher config backup. User passwords and active sessions are not changed.</p><div class="field"><label>Backup JSON</label><textarea id="restore-backup-json" rows="8" placeholder='{"format":"home-lab-launcher-config-v1",...}'></textarea></div><div class="inline-controls"><button class="ghost" id="preview-restore" type="button">Preview restore</button><button class="ghost danger" id="restore-backup" type="button">Restore backup</button></div><div id="restore-preview" class="restore-preview"></div></div>`;
}
